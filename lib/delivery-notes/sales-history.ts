...
export async function fetchClientSubProductSalesByYear(
  clientId: string,
  companyId: string,
  ...
  const result = new Map<string, ProductSalesByYear>();
  if (subProductIds.length === 0 || years.length === 0) return result;
  ...
  const { data: updates, error } = await supabase
    .from('stock_updates')
    .select('sub_product_id, stock_sold, created_at, invoice_id')
    .eq('client_id', clientId)
    .eq('company_id', companyId)
  ...
  for (const update of updates || []) {
    if (!update.sub_product_id || !update.invoice_id) continue;
    if (!completedIds.has(update.invoice_id)) continue;

    const year = new Date(update.created_at).getFullYear();
    if (year < minYear || year > maxYear) continue;

    const entry = result.get(update.sub_product_id);
    if (!entry || !years.includes(year)) continue;
    entry[year] += update.stock_sold || 0;
  }

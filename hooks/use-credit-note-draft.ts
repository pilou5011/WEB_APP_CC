...
export function useCreditNoteDraft(clientId: string, isActiveTab: boolean = true) {
  ...
  const saveDraftToServer = useCallback(async (data: DraftCreditNoteData) => {
    ...
    if (!isActiveTab) {
      console.log('[Draft Credit Note] Not saving: not on active tab');
      return;
    }
  ...
  const autoSave = useCallback((data: DraftCreditNoteData) => {
    // Only save if we're on the active tab
    if (!isActiveTab) {
      console.log('[Draft Credit Note] AutoSave: Not saving - not on active tab');
      return;
    }
    ...
    saveDraftToServer(data).catch(err => 
      console.error('[Draft Credit Note] Error in immediate server save:', err)
    );

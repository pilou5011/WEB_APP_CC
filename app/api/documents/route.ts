...
export async function GET(request: NextRequest) {
  try {
    ...
    const documentTypes: DocumentTypeParam[] = documentTypesParam
      ...
      : ['invoice', 'stock_report', 'deposit_slip', 'credit_note', 'delivery_note'];
    ...
    if (documentTypes.includes('delivery_note')) {
      let query = supabase
        .from('delivery_notes')
        .select(
          'id, client_id, delivery_number, validated_at, created_at, pdf_path, status, deleted_at, clients!inner(name)'
        )
      ...
      const { data: deliveryNotes, error: dnError } = await query.order('created_at', {
        ascending: false,
      });
